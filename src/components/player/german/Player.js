...
class Player {
  ...
  createModel() {
      console.log(`DEBUG: Creating high-fidelity LEGO German soldier minifigure for player ${this.id}`);
      ...
      try {
          ...
          const createStubbleDot = (x, y) => {
              ...
              const dot = new THREE.Mesh(dotGeometry, new THREE.MeshPhongMaterial({
                  color: 0x8B4513,
                  specular: 0x222222,
                  shininess: 10
              }));
          ...
          const stubblePositions = [
              {x: 0.06, y: 1.42}, {x: 0.03, y: 1.43}, {x: 0.09, y: 1.43},
              {x: -0.06, y: 1.42}, {x: -0.03, y: 1.43}, {x: -0.09, y: 1.43},
              {x: 0.05, y: 1.41}, {x: -0.05, y: 1.41}, {x: 0, y: 1.41},
              {x: 0.08, y: 1.44}, {x: -0.08, y: 1.44}, {x: 0.02, y: 1.42},
              {x: -0.02, y: 1.42}
          ];
          ...
          // ====== HELMET ======
          // Main helmet shape
          const helmetGeometry = new THREE.SphereGeometry(0.18, 16, 12, 0, Math.PI * 2, 0, Math.PI * 0.6);
          ...
          const helmetSideLeft = new THREE.Mesh(helmetSideLeftGeometry, helmetMaterial);
          ...
          const helmetSideRight = new THREE.Mesh(helmetSideRightGeometry, helmetMaterial);
          ...
          const suspenderLeft = new THREE.Mesh(suspenderLeftGeometry, detailMaterial);
          ...
          const suspenderRight = new THREE.Mesh(suspenderRightGeometry, detailMaterial);
          ...
          const hipConnection = new THREE.Mesh(hipConnectionGeometry, uniformMaterial);
          ...
          const shoulderConnectorGeometry = new THREE.CylinderGeometry(0.035, 0.035, 0.05, 8);
          
          const leftShoulderConnector = new THREE.Mesh(shoulderConnectorGeometry, uniformMaterial);
          ...
          const rightShoulderConnector = new THREE.Mesh(shoulderConnectorGeometry, uniformMaterial);
          ...
          if (!this.isLocal) {
              ...
              console.log('DEBUG: Added detailed LEGO-style Kar98k rifle to player model');
          }
          ...
          if (this.game && this.game.scene) {
              ...
              console.log(`DEBUG: High-fidelity LEGO minifigure created for player ${this.id} at position:`, this.model.position);
          } else {
              console.error(`DEBUG: Cannot add model to scene - scene not available`);
          }
      } catch (error) {
          console.error(`ERROR: Failed to create LEGO minifigure for player ${this.id}:`, error);
      }
  ...
  createSimpleRifle() {
      console.log(`DEBUG: Creating simple LEGO-style Kar98k rifle for player ${this.id}`);
      ...
      try {
          ...
          const metalMaterial = new THREE.MeshLambertMaterial({ color: metalColor });
          ...
          if (this.model) {
              this.model.add(rifleGroup);
              this.rifle = rifleGroup;
              console.log(`DEBUG: Added simple LEGO-style rifle to player ${this.id}`);
          } else {
              console.error(`DEBUG: Cannot add rifle - model not available for player ${this.id}`);
          }
          
          return rifleGroup;
      } catch (error) {
          console.error(`ERROR: Failed to create simple rifle for player ${this.id}:`, error);
          return null;
      }
  }
